"""Based on 08_initialise_snack_lists
Creates a data frame (copied from 00_MMF_v8) to present the snack data tidily
"""

import pandas

names = ["Jad", "Dan", "Jam", "Con", "Jug"]
...
test_data = [
    ...
    [[]],   # This is for Dan's order - he ordered nothing
    [[1, 'Water']],
...
for client_order in test_data:  # Going through each order
    ...
    for item in snack_order:  # The item only has 2 parts - number and snack
        if len(item) > 0:  # Checking to eliminate any blank orders
            ...
            amount = item[0]  # and sets 'count' to number ordered
